...
void loadImage() // Passed the filepath string as an argument
{
	int channels;
	stbi_set_flip_vertically_on_load(true); //flips the image because it loads it upsidedown

	//image = stbi_load("box.bmp", &width, &height, &channels, 0);

	image = stbi_load("Texture.jpg", &width, &height, &channels, 0);
...
int main() {
	...
	// Texture Parameters
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR); //GL Linear
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR); //GL Linear
